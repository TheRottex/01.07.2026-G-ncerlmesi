...
public sealed class VortexDb
{
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken = default)
    {
        ...
        await ApplyLightweightMigrationsAsync(connection, cancellationToken);
    }
    ...
    private static async Task SeedPlansAsync(SqliteConnection connection, CancellationToken cancellationToken)
    {
        ...
        await ExecuteAsync(connection, "INSERT INTO SubscriptionPlans (Id, Name, DisplayName, StorageQuotaBytes, DailyRequestLimit, MonthlyRequestLimit, IsActive) VALUES ($id, 'free', 'Ücretsiz Plan', 5368709120, 50, 1000, 1)", cancellationToken, ("$id", freePlanId));
    ...
    private static async Task ApplyLightweightMigrationsAsync(SqliteConnection connection, CancellationToken cancellationToken)
    {
        await EnsureColumnAsync(connection, "UserAgentProfiles", "WorkerId", "TEXT NULL", cancellationToken);
        await EnsureColumnAsync(connection, "UserAgentProfiles", "WorkspaceId", "TEXT NULL", cancellationToken);
        await EnsureColumnAsync(connection, "UserAgentProfiles", "PlanId", "TEXT NULL", cancellationToken);
        await EnsureColumnAsync(connection, "UserAgentProfiles", "StorageQuotaBytes", "INTEGER NOT NULL DEFAULT 0", cancellationToken);
        await EnsureColumnAsync(connection, "UserAgentProfiles", "StorageUsedBytes", "INTEGER NOT NULL DEFAULT 0", cancellationToken);
        await EnsureColumnAsync(connection, "UserAgentProfiles", "ProfileStatus", "TEXT NULL", cancellationToken);
        await EnsureColumnAsync(connection, "UserAgentProfiles", "DisabledAt", "TEXT NULL", cancellationToken);
        await EnsureColumnAsync(connection, "UserAgentProfiles", "LastUsedAt", "TEXT NULL", cancellationToken);
        await ExecuteAsync(connection, "UPDATE SubscriptionPlans SET StorageQuotaBytes = 5368709120 WHERE Name = 'free' AND StorageQuotaBytes < 5368709120", cancellationToken);
        await ExecuteAsync(connection, "UPDATE UserAgentProfiles SET WorkspaceId = lower(hex(randomblob(16))) WHERE WorkspaceId IS NULL OR WorkspaceId = ''", cancellationToken);
        await ExecuteAsync(connection, "UPDATE UserAgentProfiles SET ProfileStatus = Status WHERE ProfileStatus IS NULL OR ProfileStatus = ''", cancellationToken);
        await ExecuteAsync(connection, "UPDATE UserAgentProfiles SET PlanId = (SELECT PlanId FROM Users WHERE Users.Id = UserAgentProfiles.UserId) WHERE PlanId IS NULL", cancellationToken);
        await ExecuteAsync(connection, "UPDATE UserAgentProfiles SET StorageQuotaBytes = COALESCE((SELECT StorageQuotaBytes FROM SubscriptionPlans WHERE SubscriptionPlans.Id = UserAgentProfiles.PlanId), 5368709120) WHERE StorageQuotaBytes = 0", cancellationToken);
    }
    ...
    private static async Task EnsureColumnAsync(SqliteConnection connection, string table, string column, string definition, CancellationToken cancellationToken)
    {
        ...
        if (!exists) await ExecuteAsync(connection, $"ALTER TABLE {table} ADD COLUMN {column} {definition}", cancellationToken);
    }
    ...
    [
        ...
        "CREATE TABLE IF NOT EXISTS UserAgentProfiles (Id TEXT PRIMARY KEY, UserId TEXT NOT NULL UNIQUE, HermesProfileName TEXT NOT NULL UNIQUE, HermesHomePath TEXT NOT NULL, Status TEXT NOT NULL, CreatedAt TEXT NOT NULL, LastStartedAt TEXT NULL, WorkerId TEXT NULL, WorkspaceId TEXT NULL, PlanId TEXT NULL, StorageQuotaBytes INTEGER NOT NULL DEFAULT 0, StorageUsedBytes INTEGER NOT NULL DEFAULT 0, ProfileStatus TEXT NULL, DisabledAt TEXT NULL, LastUsedAt TEXT NULL, FOREIGN KEY (UserId) REFERENCES Users(Id) ON DELETE CASCADE);",
        "CREATE TABLE IF NOT EXISTS WorkerRegistrations (WorkerId TEXT PRIMARY KEY, DisplayName TEXT NOT NULL, SecretHash TEXT NULL, Status TEXT NOT NULL, LastHeartbeatAt TEXT NULL, HermesReady INTEGER NOT NULL DEFAULT 0, ModelReady INTEGER NOT NULL DEFAULT 0, StorageHealthy INTEGER NOT NULL DEFAULT 0, Message TEXT NULL, CreatedAt TEXT NOT NULL, UpdatedAt TEXT NOT NULL);",
        "CREATE TABLE IF NOT EXISTS WorkerReplayNonces (WorkerId TEXT NOT NULL, Nonce TEXT NOT NULL, Timestamp TEXT NOT NULL, ExpiresAt TEXT NOT NULL, PRIMARY KEY (WorkerId, Nonce));",
        "CREATE INDEX IF NOT EXISTS IX_WorkerReplayNonces_ExpiresAt ON WorkerReplayNonces(ExpiresAt);",
        "CREATE TABLE IF NOT EXISTS AgentJobs (Id TEXT PRIMARY KEY, UserId TEXT NOT NULL, AgentProfileId TEXT NOT NULL, ConversationId TEXT NULL, RequestId TEXT NOT NULL, IdempotencyKey TEXT NULL, Status TEXT NOT NULL, Priority INTEGER NOT NULL, Input TEXT NOT NULL, CreatedAt TEXT NOT NULL, ClaimedAt TEXT NULL, StartedAt TEXT NULL, CompletedAt TEXT NULL, LeaseExpiresAt TEXT NULL, AttemptCount INTEGER NOT NULL, MaxAttempts INTEGER NOT NULL, ErrorCode TEXT NULL, WorkerId TEXT NULL, Result TEXT NULL, CancellationRequested INTEGER NOT NULL DEFAULT 0, LastHeartbeatAt TEXT NULL, UNIQUE(UserId, IdempotencyKey), FOREIGN KEY (UserId) REFERENCES Users(Id) ON DELETE CASCADE, FOREIGN KEY (AgentProfileId) REFERENCES UserAgentProfiles(Id) ON DELETE CASCADE);",
        "CREATE INDEX IF NOT EXISTS IX_AgentJobs_Status_Lease ON AgentJobs(Status, LeaseExpiresAt, Priority, CreatedAt);",
        "CREATE INDEX IF NOT EXISTS IX_AgentJobs_User ON AgentJobs(UserId, CreatedAt);",
        ...
        "CREATE TABLE IF NOT EXISTS DesktopAuthSessions (Id TEXT PRIMARY KEY, StateHash TEXT NOT NULL, CodeChallenge TEXT NOT NULL, CallbackUri TEXT NOT NULL, UserId TEXT NULL, AuthorizationCodeHash TEXT NULL, ExpiresAt TEXT NOT NULL, CompletedAt TEXT NULL, ConsumedAt TEXT NULL, CreatedAt TEXT NOT NULL, FOREIGN KEY (UserId) REFERENCES Users(Id) ON DELETE CASCADE);",
