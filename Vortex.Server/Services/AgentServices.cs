...
public sealed class AgentIsolationService(IConfiguration configuration, IWebHostEnvironment environment) : IAgentIsolationService
{
    ...
    public bool IsInsideUserWorkspace(Guid userId, string fullPath)
    {
        ...
        return target.Equals(workspace, comparison) || target.StartsWith(workspace + Path.DirectorySeparatorChar, comparison);
    }
}
...
public sealed class HermesProfileService(VortexDb db, IConfiguration configuration, ILogger<HermesProfileService> logger) : IHermesProfileService
{
    public async Task<AgentProfileDto> EnsureProfileAsync(Guid userId, CancellationToken cancellationToken)
    {
        ...
        var planId = Convert.ToString(await ScalarAsync(connection, "SELECT PlanId FROM Users WHERE Id = $userId", cancellationToken, ("$userId", userId.ToString()))) ?? throw new InvalidOperationException("Kullanıcı planı bulunamadı.");
        var quota = await VortexDb.ScalarLongAsync(connection, "SELECT StorageQuotaBytes FROM SubscriptionPlans WHERE Id = $planId", cancellationToken, ("$planId", planId));
        var workspaceId = Guid.NewGuid().ToString("N");
        var workerId = configuration["Worker:AllowedWorkerId"];
        var profile = new AgentProfileDto(Guid.NewGuid(), userId, $"hermes-{workspaceId}", workspaceId, HermesProfileStatus.Ready.ToString(), now, null, workerId, workspaceId, quota, 0);
        await VortexDb.ExecuteAsync(connection, "INSERT INTO UserAgentProfiles (Id, UserId, HermesProfileName, HermesHomePath, Status, CreatedAt, LastStartedAt, WorkerId, WorkspaceId, PlanId, StorageQuotaBytes, StorageUsedBytes, ProfileStatus, DisabledAt, LastUsedAt) VALUES ($id, $userId, $name, $home, $status, $createdAt, NULL, $workerId, $workspaceId, $planId, $quota, 0, $status, NULL, NULL)", cancellationToken, ("$id", profile.Id.ToString()), ("$userId", userId.ToString()), ("$name", profile.HermesProfileName), ("$home", profile.HermesHomePath), ("$status", profile.Status), ("$createdAt", now.ToString("O")), ("$workerId", workerId), ("$workspaceId", workspaceId), ("$planId", planId), ("$quota", quota));
    ...
    public async Task<AgentProfileDto?> GetProfileAsync(Guid userId, CancellationToken cancellationToken)
    {
        ...
        command.CommandText = "SELECT Id, UserId, HermesProfileName, HermesHomePath, Status, CreatedAt, LastStartedAt, WorkerId, WorkspaceId, StorageQuotaBytes, StorageUsedBytes FROM UserAgentProfiles WHERE UserId = $userId";
        command.Parameters.AddWithValue("$userId", userId.ToString());
        await using var reader = await command.ExecuteReaderAsync(cancellationToken);
        if (!await reader.ReadAsync(cancellationToken)) return null;
        return new AgentProfileDto(Guid.Parse(reader.GetString(0)), Guid.Parse(reader.GetString(1)), reader.GetString(2), reader.GetString(3), reader.GetString(4), DateTimeOffset.Parse(reader.GetString(5)), reader.IsDBNull(6) ? null : DateTimeOffset.Parse(reader.GetString(6)), reader.IsDBNull(7) ? null : reader.GetString(7), reader.IsDBNull(8) ? null : reader.GetString(8), reader.GetInt64(9), reader.GetInt64(10));
    }
    ...
    private async Task TryProvisionAsync(AgentProfileDto profile, CancellationToken cancellationToken)
    {
        ...
        try
        {
            if (string.IsNullOrWhiteSpace(profile.WorkspaceId)) throw new InvalidOperationException("WorkspaceId oluşturulamadı.");
        }
        catch (Exception ex)
        {
            status = HermesProfileStatus.ProvisioningFailed.ToString();
            logger.LogWarning(ex, "Hermes profile mapping failed for {UserId}; secrets redacted", profile.UserId);
        }
        await using var connection = await db.OpenAsync(cancellationToken);
        await VortexDb.ExecuteAsync(connection, "UPDATE UserAgentProfiles SET Status = $status, ProfileStatus = $status, LastStartedAt = $lastStartedAt WHERE Id = $id", cancellationToken, ("$status", status), ("$lastStartedAt", DateTimeOffset.UtcNow.ToString("O")), ("$id", profile.Id.ToString()));
    }
