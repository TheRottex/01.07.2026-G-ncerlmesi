    ...
    CancellationToken cancellationToken)
{
    ...
    // Tam cevap: JSON parse etmek için kullanılır.
    var responseBody = await response.Content.ReadAsStringAsync(cancellationToken);

    // Kısaltılmış cevap: yalnızca log ve hata mesajlarında kullanılır.
    var safeBody = responseBody.Length > 500
        ? responseBody[..500] + "..."
        : responseBody;
    ...
    if (response.IsSuccessStatusCode)
    {
        ...
        auth = JsonSerializer.Deserialize<AuthResponse>(
            responseBody,
            JsonOptions);
